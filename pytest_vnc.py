...
class VNC:
	...
	@classmethod
	def connect(cls, host: str, port: int, password: str, pixel_format: str = 'rgba') -> 'VNC':
		"""
		Establishes a connection to a VNC server, sends the password, and returns a VNC client object.
		"""
		# Connect and handshake
		...
		if auth_type == 33:
			...
			sock.sendall(
				b'\x00\x00\x01\x8a\x01\x00RSA1' +
				b'\x00\x01' + encryptor.update(pack_ard('') + pack_ard(password)) +
				b'\x00\x01' + host_key.encrypt(aes_key, padding=PKCS1v15()))
			read(sock, 4)  # padding
		...
		return cls(sock, decompressobj().decompress, 20.0, rect)
